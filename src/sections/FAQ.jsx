...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="relative py-24 bg-white  transition-colors duration-500 scroll-mt-16 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="glass border border-navy-deep/5  rounded-2xl overflow-hidden transition-all duration-300 shadow-sm"
              >
                {/* Trigger Button */}
                <button
                  onClick={() => toggleFAQ(idx)}
                  className="w-full px-6 py-5 flex items-center justify-between text-left font-extrabold text-sm sm:text-base text-navy-deep  transition-colors hover:text-royal :text-royal-light cursor-pointer select-none"
                >
                ...
                >
                  <div className="overflow-hidden">
                    <div className="px-6 pb-5 pt-1 text-xs sm:text-sm text-slate-500  leading-relaxed font-semibold border-t border-navy-deep/5 ">
                      {faq.a}
                    </div>
                  </div>
                </div>
        ...
        {/* FAQ Contact Prompt */}
        <div className="mt-12 p-6 glass-card border border-white/20  text-center flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 text-left">
            <div className="w-10 h-10 rounded-full bg-royal/10  text-royal flex items-center justify-center shrink-0">
              <MessageCircle className="w-5 h-5 text-gold" />
            </div>
            <div>
              <h4 className="font-extrabold text-sm text-navy-deep ">Still have questions?</h4>
